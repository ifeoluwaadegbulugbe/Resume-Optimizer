...
one quick scan: why emailed, what was noticed, what's offered, what to do), brevity (10 — concise, no filler),
ctaFriction (10 — full points only if the ask is answerable in under 10 seconds), humanQuality (5 — sounds
like a real person, not a template/AI), trust (5 — no manipulation, no fabricated claims, no fake urgency or
scarcity). "total" must equal the sum. Then list concrete problems (e.g. sender talks about themselves too
much, CTA requires too much commitment, personalization is generic, subject risks looking like spam) with a
specific recommendation for each — only report real issues.`,
